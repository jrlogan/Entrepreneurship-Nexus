...
// ----------------------------------------------------------------------------
const normalizeForHash = (s) => s.toLowerCase().replace(/[^a-z0-9]/g, '').slice(0, 200);
const eventFingerprint = (title, startTime, locationText) => {
    ...
    for (const line of lines) {
        ...
        switch (key) {
            ...
            case 'ORGANIZER': {
                ...
                if (cn)
                    cur.organizer_name = cn[1];
                const mail = value.match(/mailto:(.+)/i);
                if (mail)
                    cur.organizer_email = mail[1];
                break;
            }
        }
    }
...
const parseRss = (xml) => {
    ...
    while ((match = itemRe.exec(xml)) !== null) {
        ...
        if (!startIso)
            continue; // RSS items without dates can't anchor on a calendar
        items.push({
...
// ----------------------------------------------------------------------------
const buildClassifyPrompt = (candidate, ctx) => `You are an event classification assistant for an entrepreneurship ecosystem calendar.
...
const classifyEventCandidate = async (candidate, ctx) => {
        ...
        try {
            ...
            return {
                ...
                suggested_tags: Array.isArray(parsed.suggested_tags)
                    ...
                    : [],
                scope: (['local', 'regional', 'state', 'national'].includes(parsed.scope) ? parsed.scope : 'local'),
...
exports.classifyEventCandidate = classifyEventCandidate;
// ----------------------------------------------------------------------------
// Core ingest: dedup, classify, route to status, write event.
// ----------------------------------------------------------------------------
const decideStatus = (source, classification) => {
    if (source.filter_mode === 'trust')
    ...
    if (classification.confidence >= threshold && classification.is_entrepreneurial)
        return 'auto_approved';
    if (classification.confidence >= PENDING_REVIEW_FLOOR)
        return 'pending_review';
    return 'rejected';
};
const findExistingEvent = async (source_id, source_event_id, fingerprint) => {
...
const ingestEventCandidate = async (candidate, ctx) => {
    const fingerprint = (0, exports.eventFingerprint)(candidate.title, candidate.start_time, candidate.location_text);
    ...
    if (ctx.source?.filter_mode !== 'trust') {
        classification = await (0, exports.classifyEventCandidate)(candidate, {
    ...
    const tags = Array.from(new Set([...(ctx.source?.default_tags || []), ...(classification?.suggested_tags || [])]));
    const geographic_tags = Array.from(new Set([
        ...(ctx.source?.default_geographic_tags || []),
        ...(classification?.geographic_tags || []),
    ]));
    const scope = classification?.scope || ctx.source?.default_scope || 'local';
    const visibility = ctx.source?.default_visibility || 'public';
    const eventDoc = {
        id,
...
exports.ingestEventCandidate = ingestEventCandidate;
// ----------------------------------------------------------------------------
// Source polling
// ----------------------------------------------------------------------------
...
const recordSourceRun = async (source, startedAt, result) => {
...
exports.pollEventSources = (0, scheduler_1.onSchedule)({ schedule: 'every 4 hours', timeoutSeconds: 540, memory: '512MiB' }, async () => {
    const result = await pollAllSources();
    console.log('[calendar] poll complete', result);
});
// Manual trigger for ops/testing.
exports.triggerEventSourcePoll = (0, https_1.onRequest)({ invoker: 'public' }, async (req, res) => {
...
// ----------------------------------------------------------------------------
const processCalendarSubmissionEmail = async (args) => {
...
JSON only.`;
    ...
    if (!candidate.start_time)
        return { ok: false };
    const result = await (0, exports.ingestEventCandidate)(candidate, {
        source: null,
...
exports.submitEventUrl = (0, https_1.onCall)({ timeoutSeconds: 60 }, async (request) => {
    if (!request.auth)
        throw new https_1.HttpsError('unauthenticated', 'Sign in to submit events');
...
// ----------------------------------------------------------------------------
const escapeIcal = (s) => s.replace(/\\/g, '\\\\').replace(/\n/g, '\\n').replace(/,/g, '\\,').replace(/;/g, '\\;');
...
const buildIcalFeed = (events, feedName) => {
    const lines = [];
    ...
    for (const ev of events) {
        ...
        if (ev.description)
            lines.push(foldIcalLine(`DESCRIPTION:${escapeIcal(ev.description)}`));
        if (ev.location?.text)
            lines.push(foldIcalLine(`LOCATION:${escapeIcal(ev.location.text)}`));
        if (ev.url)
            lines.push(foldIcalLine(`URL:${ev.url}`));
        lines.push('END:VEVENT');
    }
    lines.push('END:VCALENDAR');
    return lines.join('\r\n');
};
exports.generateCalendarFeed = (0, https_1.onRequest)({ invoker: 'public' }, async (req, res) => {
    // Path: /generateCalendarFeed?ecosystem=ID&tags=funding,pitch&scope=local
    // Or: /generateCalendarFeed (platform aggregate)
    const ecosystemId = req.query.ecosystem || '';
    const tagsCsv = req.query.tags || '';
...
// Flagging
// ----------------------------------------------------------------------------
exports.flagEvent = (0, https_1.onCall)(async (request) => {
    ...
    if (!event_id || !flag_type)
        throw new https_1.HttpsError('invalid-argument', 'event_id and flag_type required');
    const allowed = ['wrong_date', 'wrong_location', 'not_relevant', 'duplicate', 'other'];
    if (!allowed.includes(flag_type))
        throw new https_1.HttpsError('invalid-argument', 'invalid flag_type');
...
exports.resolveEventFlag = (0, https_1.onCall)(async (request) => {
    if (!request.auth)
    ...
    if (!['resolved', 'dismissed'].includes(decision)) {
        throw new https_1.HttpsError('invalid-argument', 'decision must be resolved|dismissed');
    }
    const flagDoc = await db().collection('event_flags').doc(flag_id).get();
    if (!flagDoc.exists)
        throw new https_1.HttpsError('not-found', 'flag not found');
    const eventId = flagDoc.data()?.event_id;
    const now = new Date().toISOString();
