...
exports.enforceRateLimit = exports.consumeRateLimit = exports.PARTNER_RULES = void 0;
/**
 ...
 */
const consumeRateLimit = async (db, bucket, ruleName, rule) => {
    const now = Date.now();
    ...
    try {
        count = await db.runTransaction(async (tx) => {
            ...
            const current = (snap.exists ? snap.get('count') : 0) || 0;
    ...
    if (rule.alertAt && count === Math.ceil(rule.limit * rule.alertAt)) {
        ...
        console.warn(`RATE_LIMIT_APPROACHING rule=${ruleName} bucket=${bucket} count=${count}/${rule.limit}`);
 ...
 */
const enforceRateLimit = async (db, bucket, ruleName, res) => {
    const rule = exports.PARTNER_RULES[ruleName];
    const result = await (0, exports.consumeRateLimit)(db, bucket, ruleName, rule);
